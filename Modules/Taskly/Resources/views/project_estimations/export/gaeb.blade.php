...
<GAEB xmlns="http://www.gaeb.de/GAEB_DA_XML/DA83/3.2">
    <PrjInfo>
        <Cur>{{ $money_format }}</Cur>
    </PrjInfo>
    <Award>
        <BoQ>
            <BoQBody>
@php $i = 1; @endphp
@foreach($estimation_result as $erow)
...
@php
$single_price = isset($quote_items[$row->id]->price) ? export_money_format($quote_items[$row->id]->price) : 0;
$item_total_price = isset($quote_items[$row->id]->total_price) ? export_money_format($quote_items[$row->id]->total_price) : 0;
@endphp
