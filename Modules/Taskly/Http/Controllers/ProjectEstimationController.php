...
class ProjectEstimationController extends Controller
{
    ...
    private function process_excel_file(Request $request, $file)
    {
        ...
        foreach ($worksheet_arr as $index => $row) {
            ...
            if (count(array_intersect(['quantity', 'menge', 'qty', 'anzahl'], $lowercasedRow)) > 0)
                $matchedHeaders++;
            if (count(array_intersect(['unit', 'me', 'einheit', 'mengeneinheit'], $lowercasedRow)) > 0)
                $matchedHeaders++;
            if (count(array_intersect(['title', 'kurztext', 'titel'], $lowercasedRow)) > 0)
                $matchedHeaders++;
            if (count(array_intersect(['single_price', 'ep', 'single', 'Einzelpreis'], $lowercasedRow)) > 0)
                $matchedHeaders++;
            if (count(array_intersect(['Optional', 'Eventualposition', 'EV', 'Optionalposition', 'Optional Position'], $lowercasedRow)) > 0)
                $matchedHeaders++;
            if (count(array_intersect(['description', 'beschreibung', 'desc', 'langtext'], $lowercasedRow)) > 0)
                $matchedHeaders++;
    ...
    public function finalizeEstimation($id)
    {
        ...
        return view("taskly::project_estimations.finalize", compact(
            'estimation',
            'quote',
            'settings',
            'estimateEmailTemplate',
            'estimatePdfTopTemplate',
            'estimatePdfEndTemplate',
            'site_money_format',
            'additional_files_list',
    ...
    public function changeStatus(ProjectEstimation $projectEstimation, $statusID)
    {
        $user = Auth::user();
        if (! $user->isAbleTo('estimation manage')) {
            return response(['status' => false, 'message' => __("Permission Denied.")]);
        ...
        $projectEstimation->status = $statusID;
        $projectEstimation->save();

        return redirect()->back()->with('success', __('Status has been successfully changed'));
    }
}
