...
@section('content')
    ...
    <div class="row">
        ...
        <div class="nav nav-tabs project_tabs" id="status-tabs" role="tablist">
            <a class="nav-item nav-link active project_status_link" id="all-tab" data-toggle="tab" href="#all"
                role="tab" aria-controls="all" aria-selected="true">{{ __('All') }}</a>
            @if (isset($project_dropdown['project_status']) && !empty($project_dropdown['project_status']))
                @foreach ($project_dropdown['project_status'] as $status)
                    ...
                    @endphp
                    <a class="nav-item nav-link project_status_link" id="status-tab-{{ $status->id }}" data-toggle="tab"
                        href="#status-{{ $status->id }}" role="tab" aria-controls="status-{{ $status->id }}"
                        aria-selected="false"
                            ...
                            <thead>
                                <tr class="form-group">
                                    ...
                                    <th><input type="text" class="form-control form_filter_field"
                                            placeholder="{{ __('Comments') }}" name="filter_comment"
                                            id="filter_comment"></th>
                                    <th>
                                    ...
                                    <th class="range_slider">
                                        <div class="price-input price_input">
                                            <div class="field">
                                                <input type="number" id="filter_price_from"
                                                    class="input-min form_filter_field" value="0">
                                            </div>
                                            <div class="separator">-</div>
                                            <div class="field">
                                                <input type="number" id="filter_price_to"
                                                    class="input-max form_filter_field" value="{{ $half_price }}">
                                            </div>
                                        </div>
                                        ...
                                        <div class="range-input price_range_input">
                                            <input type="range" class="range-min" min="0"
                                                max="{{ $projectmaxprice }}" value="0" step="10"
                                                onmouseup="set_filter_values()">
                                            <input type="range" class="range-max" min="0"
                                                max="{{ $projectmaxprice }}" value="{{ $half_price }}"
                                                onmouseup="set_filter_values()" step="10">
                                        </div>
                                    </th>
                                    <th>
                                ...
                                <tr>
                                    ...
                                    <th scope="col" class="sort project-priority" data-sort="priority">
                                        {{ __('Priority') }}</th>
                                    <th scope="col" class="sort project-construction-type"
                                        data-sort="construction_type">{{ __('Construction') }}</th>
    ...
    <script>
        ...
        function load_projects() {
            if ($("#pc-dt-simple-project").length > 0) {
                ...
                const projectsTableData = ('#pc-dt-simple-project').DataTable({
                    "language": {
                    ...
                    "columns": [{
                        ...
                        {
                            "data": "construction",
                            "className": "project-construction-type"
                        },
                        ...
                        //{ "data": "users", "orderable": false, "className": "project-users" },
                        {
                            "data": "action",
                            "orderable": false,
                            "className": "project-action text-end"
                        },
                    ],
                    "columnDefs": [{
                            'targets': 3,
                    ...
                    'fnDrawCallback': function(oSettings) {
                        ...
                        $('.dataTables_filter').each(function() {
                            ...
                            if (filter_data['project_type'] == 'archieve') {
                                delete_button_html +=
                                    '<a href="javascript:void(0)" class="btn btn-sm btn-primary action_projects" data-type="remove_archieve"><i class="ti ti-archive text-white"></i> Remove from Archive</a>';
                            } else {
                                delete_button_html +=
                                    '<a href="javascript:void(0)" class="btn btn-sm btn-primary action_projects" data-type="archieve"><i class="ti ti-archive text-white"></i> Move to Archive</a>';
                            }
                            delete_button_html +=
                                '<a href="javascript:void(0)" class="btn btn-sm btn-primary action_projects" data-type="delete"><i class="ti ti-trash text-white"></i> Delete Projects</a>';
        ...
        function get_dates_from_date_range_picker() {
            if ($('.daterange').length > 0) {
                ...
                if (date_range != "") {
                    if (date_range == "Today" || date_range == "Yesterday" || date_range == "This Week" || date_range ==
                        "This Month") {
                        if (date_range == "Today") {
