                                        ...
                                        @foreach ($row['estimations_list'] as $erow)
                                            @if (!empty($erow['estimation_queues_list']))
                                                @foreach ($erow['estimation_queues_list'] as $qrow)
                                                    <tr>
                                                        <td>{{ $qrow['created_at'] }}</td>
                                                        <td>
                                                            <div class="project_block" data-id="{{ $qrow['project_id'] }}">
                                                                <div class="estimation_block d-flex"
                                                                    data-id="{{ $erow['estimation_id'] }}">
                                                                    @php
                                                            ...
                                                            @if ($qrow['completed_percentage'] < 100 && $qrow['pending_record'] > 0)
                                                                <div class="">
                                                                    {!! Form::open([
                                                                        'method' => 'POST',
                                                                        'route' => ['estimations.cancel_queue', \Crypt::encrypt($erow['estimation_id'])],
                                                                    ]) !!}
                                                        ...
                                                        <td><a
                                                                href="{{ route('project.show', [$key]) }}">{{ $row['project_title'] }}</a>
                                                        </td>

                                                        <td><a
                                                                href="{{ route('estimations.setup.estimate', \Crypt::encrypt($erow['estimation_id'])) }}">{{ $erow['estimation_title'] }}</a>
                                                        </td>
