        ...
        <tr class="group group_row" :data-id="group.id" :data-groupid="group.id" data-type="group">
            ...
            <td colspan="4" class="column_name grouptitle border-right">
                <div class="div-desc-toggle">
                    <input 
                        type="text" 
                        class="form-control grouptitle-input" 
                        :value="group?.name || 'New Group'"
                        :name="`item[${group.id}][group]`" 
                        @blur="handleInputBlur($event, 'group')"
                    >
                </div>
            </td>
            @foreach ($allQuotes as $quotes)
                <td colspan="2" 
                    class="text-right grouptotal border-left-right" 
                    data-cardquoteid="{{ $quotes->id }}"
                    x-text="formatCurrency(group?.total || 0)"
                ></td>
            @endforeach
        </tr>
        ...
        <template x-for="entry in getSortedItemsForGroup(group.id)" :key="entry.id">
            <template x-if="true">
                <tr :class="{'item_row': entry.type === 'item', 'item_comment': entry.type === 'comment'}"
                    :data-id="entry.id" 
                    :data-itemid="entry.type === 'item' ? entry.id : null"
                    :data-commentid="entry.type === 'comment' ? entry.id : null"
                        ...
                        <td class="column_name">
                            <div class="div-desc-toggle">
                                <i class="desc_toggle fa fas fa-solid"
                                    ...
                                    @click="toggleDescription(entry.id, $event)"
                                ></i>
                                <input type="text" 
                                    :name="`item[${entry.id}][name]`" 
                                    class="item-name form-control"
                                    :value="entry?.name || 'New Item'" 
                    ...
                    <td x-show="entry.type === 'item'" class="column_quantity">
                        <input type="text" 
                            class="form-control row_qty item-quantity"
                            :value="formatDecimal(entry?.quantity || 0)" 
                    ...
                    <td x-show="entry.type === 'item'" class="column_optional border-right">
                        <input type="checkbox" 
                            class="select_optional item-optional"
                            :checked="entry?.optional || false" 
                    ...
                    <template x-if="entry.type === 'comment'">
                        <td colspan="4" class="border-right column_name">
                            <input type="text" 
                                :name="`item[${entry.id}][comment]`" 
                                class="form-control item-description"
                                :value="entry?.content || 'Write your comment'" 
                    ...
                    @foreach ($allQuotes as $index => $quotes)
                        <template x-if="entry.type === 'item'">
                            <td class="column_single_price border-left" data-cardquoteid="{{ $quotes->id }}">
                                <input type="text" 
                                    class="form-control row_price item-price"
                                    :value="formatCurrency(entry?.price || 0)" 
                        ...
                        <td class="column_total_price border-right" 
                            data-cardquoteid="{{ $quotes->id }}"
                            x-text="entry.type === 'item' ? (entry?.optional ? '-' : formatCurrency(calculateItemTotal(entry.id, {{ $index }}))) : '-'"
                        ></td>
                    @endforeach
                </tr>
                ...
                <template x-if="entry.type === 'item'">
                    <tr class="item_child tr_child_description" 
                        ...
                        <td colspan="4" class="column_name desc_column">
                            <textarea 
                                class="description_input w-100 tinyMCE-DIS"
                                :name="`item[${entry.id}][description]`"
