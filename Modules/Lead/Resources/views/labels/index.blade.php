    ...
    <div class="row align-items-center m-1">
        @permission('labels create')
            <div class="col-auto pe-0">
                <a class="btn btn-sm btn-primary btn-icon m-1" data-bs-toggle="tooltip" data-bs-placement="top"
                    title="{{ __('Create Label') }}" data-ajax-popup="true" data-size="md" data-title="{{ __('Create Label') }}"
                    data-url="{{ route('labels.create') }}"><i class="ti ti-plus text-white"></i></a>
            </div>
        @endpermission
    </div>
...
@push('css')
    <link href="{{ asset('public/css/jquery.minicolors.css') }}" rel="stylesheet">
@endpush
...
@section('content')
                                    ...
                                    <ul class="list-group sortable">
                                        @foreach ($pipeline['labels'] as $label)
                                            <li class="list-group-item" data-id="{{ $label->id }}">
                                                <span class="badge fix_badges p-2 px-3 rounded"
                                                    style="background-color: {{ $label->background_color }};color:{{ $label->font_color }}">{{ $label->name }}</span>
                                                <span class="float-end">
                                                    @permission('labels edit')
                                                        <div class="action-btn bg-info ms-2">
                                                            <a data-size="md"
                                                                data-url="{{ URL::to('labels/' . $label->id . '/edit') }}"
                                                                data-ajax-popup="true" data-title="{{ __('Edit Labels') }}"
                                                                class="mx-3 btn btn-sm d-inline-flex align-items-center"
                                                                data-bs-toggle="tooltip" data-bs-placement="top"
                                                                title="{{ __('Edit Labels') }}"><i
    ...
    @push('scripts')
        <script src="{{ asset('public/js/jquery.minicolors.js') }}"></script>
    @endpush
