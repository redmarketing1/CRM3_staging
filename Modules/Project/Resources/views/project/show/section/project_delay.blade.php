                        ...
                        <div class="float-end">
                            <p class="text-muted d-none d-sm-flex align-items-center mb-0">
                                <a href="javascript:;" class="btn btn-sm btn-primary" data-ajax-popup="true" 
                                    data-title="{{ __('Add Project Delay Announcement') }}" title="{{ __('Add Project Delay Announcement') }}" data-toggle="tooltip"
                                    data-size="lg"
                                    data-url="{{ route('project.addProjectDelay', $project->id) }}"> 
                                ...
                                <td>
                                    @if($delay->media)
                                        @foreach(json_decode($delay->media) as $image)
                                            <a class="lightbox-link" href="{{ get_file($image) }}" data-lightbox="gallery" data-title="image from {{ $project->title }}">
                                                <img alt="image from {{ $project->title }}"
                                                        width="40"
                                                        height="40"
                                                        src="{{ get_file($image) }}"
                                                        class="img-thumbnail">
