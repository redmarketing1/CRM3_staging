...
@endif
@php
	$description_hidden             = '<input type="hidden" class="form-control description_input_'.$product->id.'" value="'.$product->description.'">';
...
<tr class="item_row" data-id="{{$product->id}}" data-group_id="{{$product->group_id}}" data-group_pos="{{$product->group->group_pos}}" data-type="{{$product->type}}">
	...
	<td class="column_name" style="padding-left: {{ $padding_left }}px !important">
		<div class="div-desc-toggle">
			...
			<input type="text" name="item[{{$product->id}}]['name']" class="form-control hide edit-view name_input_{{$product->id}}" value="{{$product->name}}">
		</div>
	</td>
		...
		@foreach ($quote_items[$product->id] as $quoteItem)
			...
			<td class="column_single_price border-left quote_th{{$quoteItem->estimate_quote_id}}" data-quote="{{$quoteItem->estimate_quote_id}}">
				<div class="d-flex">
					...
					<input type="text" class="form-control row_price single_price_sc{{$quoteItem->estimate_quote_id}} single_price_sc_input_{{$product->id.'_'. $quoteItem->estimate_quote_id}}" oninput="validateInput(this)" data-id="{{$quoteItem->estimate_quote_id}}" data-item_id="{{$product->id}}" value="{{currency_format_with_sym($quoteItem->price,'','',false)}}">
			...
			<td class="column_total_price border-righ quote_th{{$quoteItem->estimate_quote_id}}t tot_price_{{$product->id.'_'. $quoteItem->estimate_quote_id}}">
