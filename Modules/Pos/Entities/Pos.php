...
class Pos extends Model
{
    ...
    {
        // $sells = Pos::select('pos.*', \DB::raw('SUM(pos_products.price * (1 + pos_products.tax / 10) * pos_products.quantity) as total_price'))
        //     ->where('created_by', creatorId())->where('pos_products.workspace',getActiveWorkSpace());
        // if($month)
        // {
        //     $sells = $sells->whereRaw('MONTH(pos.created_at) = ?', [date('m')]);
        // }
        // $sells = $sells->join('pos_products', 'pos.id', '=', 'pos_products.pos_id')
        //     ->groupBy('pos.id')
        //     ->get();
        ...
        //     'pos.id',
        //     'pos.pos_date',
        //     \DB::raw('SUM(pos_products.price * (1 + pos_products.tax / 10) * pos_products.quantity) as total_price')
        // )
        // ->leftJoin('pos_products', 'pos.id', '=', 'pos_products.pos_id')
        // ->where('pos.created_by', creatorId())
        // ->where('pos_products.workspace', getActiveWorkSpace());

        // if ($month) {
        //     $sells = $sells->whereMonth('pos.pos_date', '=', [date('m')]);
        // }

        // $sells = $sells->groupBy('pos.id', 'pos.pos_date')
        //     ->get();

        // $selledAmount = 0;
        // foreach ($sells as $key => $sell) {
        //     $selledAmount += $sell->total_price;
        // }
    ...
    public static function totalTaxRate($taxes)
    {
        ...
        if(is_null(self::$taxes)){
            self::$taxes  = \Modules\ProductService\Entities\Tax::whereIn('id', explode(',', $taxes))->get();
        }
