...
var _require = __webpack_require__(/*! vue */ "./node_modules/vue/dist/vue.esm-bundler.js"),
  vModelCheckbox = _require.vModelCheckbox;
Alpine.data('estimationShow', function () {
  return {
     ...
     */
    setupCalculations: function setupCalculations() {
      var _this2 = this;
      this.$nextTick(function () {
        document.querySelectorAll('.item_row').forEach(function (row) {
          var qtyInput = row.querySelector('.row_qty');
          var priceInput = row.querySelector('.row_price');
          var optionalCheck = row.querySelector('.select_optional');
          qtyInput === null || qtyInput === void 0 || qtyInput.addEventListener('blur', function (e) {
            var rawValue = e.target.value.replace(/[^\d,-]/g, '');
            _this2.calculateRowTotal(row);
            qtyInput.value = _this2.formatDecimal(_this2.parseNumber(rawValue));
          });
          qtyInput === null || qtyInput === void 0 || qtyInput.addEventListener('keyup', function (e) {
            if (e.key === 'Enter') {
              e.target.blur();
            }
          });
          priceInput === null || priceInput === void 0 || priceInput.addEventListener('blur', function (e) {
            var rawValue = e.target.value.replace(/[^\d,-]/g, '');
            _this2.calculateRowTotal(row);
            priceInput.value = _this2.formatDecimal(_this2.parseNumber(rawValue));
          });
          priceInput === null || priceInput === void 0 || priceInput.addEventListener('keyup', function (e) {
            if (e.key === 'Enter') {
              e.target.blur();
            }
          });
          optionalCheck === null || optionalCheck === void 0 || optionalCheck.addEventListener('change', function () {
            return _this2.calculateRowTotal(row);
          });
        });
      });
    },
    calculateRowTotal: function calculateRowTotal(row) {
      var quantity = this.parseNumber(row.querySelector('.row_qty').value);
      var price = this.parseNumber(row.querySelector('.row_price').value);
      var isOptional = row.querySelector('.select_optional').checked;
      var totalCell = row.querySelector('.column_total_price');
      var total = isOptional ? 0 : quantity * price;
      totalCell.textContent = this.formatCurrency(total);
      this.updateGroupTotals();
    },
    updateGroupTotals: function updateGroupTotals() {
      var _this3 = this;
      document.querySelectorAll('.group_row').forEach(function (groupRow) {
        var groupPos = groupRow.dataset.group_pos;
        var groupTotal = 0;

        // Sum all non-optional items in this group
        document.querySelectorAll(".item_row[data-group_pos=\"".concat(groupPos, "\"]")).forEach(function (itemRow) {
          if (!itemRow.querySelector('.select_optional').checked) {
            var total = _this3.parseNumber(itemRow.querySelector('.column_total_price').textContent);
            groupTotal += total;
          }
        });

        // Update group total
        var groupTotalCell = groupRow.querySelector('.grouptotal');
        groupTotalCell.textContent = _this3.formatCurrency(groupTotal);
      });
    },
    ...
    addItem: function addItem(type) {
      ...
      if (type === 'group') {
        ...
        pos = "".concat(this.lastGroupNumber.toString().padStart(2, '0'));
        this.lastItemNumbers[this.lastGroupNumber] = 0;
      } else {
        // For items and comments
        var currentGroup = this.lastGroupNumber;
        this.lastItemNumbers[currentGroup] = (this.lastItemNumbers[currentGroup] || 0) + 1;
        pos = "".concat(currentGroup.toString().padStart(2, '0'), ".").concat(this.lastItemNumbers[currentGroup].toString().padStart(2, '0'));
      }
      var newItem = {
        id: Date.now(),
        type: type,
    ...
    removeItem: function removeItem() {
      var selectedCheckboxes = document.querySelectorAll('input[type="checkbox"].item_selection:checked');
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          selectedCheckboxes.forEach(function (element) {
            var _parentElement$datase;
            var parentElement = element.closest('tr');
            var parentID = (_parentElement$datase = parentElement.dataset.id) !== null && _parentElement$datase !== void 0 ? _parentElement$datase : null;
    ...
    getPosNumber: function getPosNumber(index) {
      var groupCounter = Math.floor(index / 100) + 1;
      var itemCounter = index % 100 + 1;
      return "".concat(groupCounter.toString().padStart(2, '0'), ".").concat(itemCounter.toString().padStart(2, '0'));
    },
    ...
    initializeSortable: function initializeSortable() {
      var _this4 = this;
      var tbody = document.querySelector("#estimation-edit-table tbody");
    ...
    initializeLastNumbers: function initializeLastNumbers() {
      ...
      var existingPosElements = document.querySelectorAll('.pos-inner');
      existingPosElements.forEach(function (element) {
        ...
        if (pos) {
          ...
          if (!_this5.lastItemNumbers[groupNumber] || itemNumber > _this5.lastItemNumbers[groupNumber]) {
            _this5.lastItemNumbers[groupNumber] = itemNumber;
          }
        }
      });
    },
    updatePOSNumbers: function updatePOSNumbers() {
      ...
      this.items.forEach(function (item, index) {
        var posNumber = _this6.getPosNumber(index);
        var row = document.querySelector("tr[data-id=\"".concat(index, "\"] .pos-inner"));
        if (row) row.textContent = posNumber;
      });
    },
    filterTable: function filterTable() {
      var filter = this.searchQuery.toUpperCase();
      var table = document.getElementById('estimation-edit-table');
    ...
    checkboxAll: function checkboxAll(value) {
      ...
      document.querySelectorAll('.group_checkbox').forEach(function (checkbox) {
        checkbox.checked = value;
      });
    }
  };
