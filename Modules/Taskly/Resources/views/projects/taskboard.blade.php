...
@section('page-action')
    <div>
        @stack('addButtonHook')
        <a href="{{ route('projecttask.list', [$project->id]) }}" class="btn btn-sm btn-primary"
            data-bs-toggle="tooltip"title="{{ __('List View') }}">
            <i class="ti ti-list text-white"></i>
        </a>
        @permission('task create')
            <a class="btn btn-sm btn-primary" data-ajax-popup="true" data-size="lg" data-title="{{ __('Create New Task') }}"
...
@endpush
@php
    $UsersData =  \Modules\Taskly\Entities\Task::getUsersData()
@endphp
@section('content')
    @if (
                                                            ...
                                                            @foreach (explode(',',$task->assign_to) as $user)
                                                                {{-- @if ($key < 3) --}}
                                                                    <img alt="image" data-bs-toggle="tooltip"
                                                                        data-bs-placement="top" title="{{ $UsersData[$user]['name'] }}"
                                                                        @if ($UsersData[$user]['avatar']) src="{{ get_file($UsersData[$user]['avatar']) }}" @else src="{{ get_file('avatar.png') }}" @endif
